...
class RepositoriesPanel(private val myProject: Project) {
    ...
    fun createRepositoriesPanel(): JPanel {
        val searchField = JTextField("test")



        val repositoriesTree = Tree(RepositoriesTreeModel(myProject, searchField))
        ...
        repositoriesTree.isEditable = false
        repositoriesTree.isRootVisible = false
        repositoriesTree.cellRenderer = object: ColoredTreeCellRenderer() {
        ...
        return panel {
            ...
            row {
                panel {
                    row {
                        JScrollPane(repositoriesTree)(CCFlags.growY)
                    }
                }()
            }
        }
    ...
    class RepositoriesTreeModel(private val myProject: Project, private val mySearchField: JTextField) : AbstractTreeModel() {
        private val repositoryPlugins: List<RepositoryPlugin>
        ...
        init {
            ...
            repositoryPlugins = workspace
                    ...
                    ?.sortedBy { repo -> repo.name } ?: emptyList()
        }
